...
{
    $CI =& get_instance();
    $CI->load->model('seting/option_model');
    return $CI->option_model->get_value($key);
}

function get_id_option($key, $value)
{
    $CI =& get_instance();
    $CI->load->model('seting/option_model');
    return $CI->option_model->get_id($key, $value);
}

function get_data_option($key = 'app_name')
{
    $CI =& get_instance();
    $CI->load->model('seting/option_model');
    return $CI->option_model->gets_value($key);
}
